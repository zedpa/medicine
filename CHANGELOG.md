...
## 2026-07-01 · 侧边栏结构重整（spec-008，对齐 Claude 网页版）
- 结构分四段：品牌（无衬线）→ `✚ 新建对话` → `最近对话` 扁平列表 → 底部账户区。
- 修复「⋯」菜单挤成「.·」：会话行列宽 `[6,1] → [0.8,0.2]` + CSS 收紧 popover 触发按钮。
- 字体自适应：侧栏标题改 Inter 无衬线（宋体仅留主区）；会话标题/账户行单行截断，
  `管理员`/长标题不再折行；选中态改柔和苔绿高亮。
- 移除「清空当前对话内容」（有历史后语义冗余，直接「新建对话」即可）；后端信息由
  expander 收敛为一行 caption（保留「直接模式」文本）。
- SDD：新增 E2E `test_sidebar_structure_claude_like`（断言结构在位、`清空`按钮=0）；
  66 单测 + 14 E2E 全绿；实景 Playwright 核验。
